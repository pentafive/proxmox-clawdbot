...
echo ""
echo -e "${INFO}${YW} ⚠️  REQUIRED: Configure Clawdbot${CL}"
echo -e "${TAB}1. Enter container: ${BGN}pct enter ${CTID}${CL}"
echo -e "${TAB}2. Run setup: ${BGN}clawdbot configure${CL}"
echo -e "${TAB}3. Restart service: ${BGN}systemctl restart clawdbot${CL}"
echo ""
echo -e "${INFO}${YW} Web Interfaces:${CL}"
echo -e "${TAB}• Clawdbot: ${BGN}http://${IP}:3003${CL} (after API key configured)"
echo -e "${TAB}• Cockpit: ${BGN}https://${IP}:9090${CL} (if installed)"
echo ""
